...
function AudienceIconCircle({ children }: { children: ReactNode }) {
  return (
    <div className="relative z-10 flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-[#FE6C0E] text-white shadow-md sm:h-[72px] sm:w-[72px]">
      {children}
    </div>
  );
}
...
export function Section07Audience() {
  ...
  return (
    <section className="bg-white py-8 sm:py-16 lg:py-20">
      <Container>
        <div className="rounded-[22px] bg-[#F9F9F9] px-4 py-7 sm:rounded-[40px] sm:px-10 sm:py-14 lg:px-14 lg:py-16">
          <h2 className="mx-auto max-w-[1120px] text-center text-balance text-[18px] font-bold leading-snug tracking-tight sm:text-3xl lg:text-4xl">
          ...
          <div className="relative mx-auto mt-7 max-w-5xl sm:mt-16">
            {/* Mobile : liste verticale avec ligne */}
            <div className="relative mx-auto max-w-sm sm:hidden">
              <div
                className="pointer-events-none absolute left-6 top-2 h-[calc(100%-16px)] w-px bg-[#FE6C0E]/35"
                aria-hidden
              />
              <div className="space-y-4">
                {personas.map((p) => (
                  <div key={p.label} className="flex items-center gap-3.5">
                    ...
                    <p className="text-[14px] font-semibold leading-snug text-black sm:text-[18px]">
                      {p.label}
                    </p>
                  </div>
                ))}
          ...
          <div className="mt-8 grid gap-[8px] sm:mt-14 lg:mt-16 lg:grid-cols-2 lg:items-stretch">
            <div className="relative min-h-[180px] overflow-hidden rounded-[18px] shadow-[0_20px_50px_-20px_rgba(0,0,0,0.18)] sm:min-h-[280px] sm:rounded-[24px] lg:min-h-[360px]">
              <Image
            ...
            <div
              className="flex flex-col justify-between rounded-[18px] p-5 shadow-[0_24px_60px_-24px_rgba(0,0,0,0.45)] sm:rounded-[24px] sm:p-10 lg:p-12"
              style={{
                background:
            ...
            >
              <div>
                <h3 className="text-[20px] font-bold leading-snug sm:text-4xl lg:text-[2.25rem]">
                  <span style={{ color: AUDIENCE_ACCENT }}>
                    Que vous vendiez{' '}
                  </span>
                  <br />
                ...
                <p className="mt-3 max-w-md text-[13px] leading-relaxed text-white/90 sm:mt-5 sm:text-[15px]">
              ...
              <DemoFlowButton
                variant="primary"
                size="lg"
                className="mt-5 rounded-[14px] text-[14px] sm:mt-10 sm:rounded-[16px] sm:text-base"
                fullWidth
                style={{
                  background:
...
function TestimonialCard({
  ...
  return (
    ...
    >
      <StarRow />
      <p className="mt-2.5 text-[13px] leading-relaxed text-[#374151] sm:mt-4 sm:text-[16px]">
        {quote}
      </p>
      <div className="mt-3.5 flex items-center gap-3 sm:mt-6">
        ...
        <div>
          ...
          <p className="text-[12px] text-[#6B7280] sm:text-[13px]">{role}</p>
        </div>
      </div>
    </article>
  );
...
export function Section08Testimonials() {
  return (
    <section
      id="temoignages"
      className="testimonials-static-mobile relative overflow-hidden bg-[#FAFAFA] py-10 sm:bg-white sm:py-20 lg:py-24"
    >
      <div
      ...
      <Container className="relative">
        ...
        <p className="mx-auto mt-2.5 max-w-2xl text-center text-[13px] leading-relaxed text-[#6B7280] sm:mt-3 md:mt-5 md:text-[18px]">
          Des particuliers, des marchands et des passionnés qui ont tous la même
          longueur d&apos;avance.
        </p>

        <div className="mx-auto mt-5 flex items-center justify-center gap-3 sm:mt-8 sm:gap-4">
          <div className="flex -space-x-2" aria-hidden>
            {[
              ...
              <div
                key={i}
                className="relative h-9 w-9 overflow-hidden rounded-full border-2 border-white sm:h-11 sm:w-11"
              >
                <Image
                  src={src}
                  alt={`Utilisateur ${i + 1}`}
          ...
          <div className="flex flex-col items-start gap-0.5 sm:flex-row sm:items-center sm:gap-2">
            <StarRow n={4} />
            <span className="text-[13px] font-bold text-[#1A1A1A] sm:text-[15px]">
              +500 utilisateurs
            </span>
          </div>
        </div>
            ...
            {[
              {
                ...
                items: [
                  ...
                  {
                    quote:
                      "En moins d'une semaine, j'avais déjà trouvé un véhicule avec plus de 2 000 CHF de marge potentielle. L'outil parle vraiment aux professionnels.",
                    name: 'Thomas R.',
                    role: 'Négociant auto, Lausanne',
                  },
                  {
                    quote:
                      "CarPulse m'a permis de structurer mon activité. Je ne cherche plus au hasard, je reçois les bonnes opportunités directement.",
                    name: 'Karim B.',
              ...
              {
                duration: 48,
                className: 'testimonials-marquee-col md:mt-14',
                items: [
                  {
                    quote:
                  ...
                  {
                    quote:
                      'La comparaison avec le prix du marché est ultra précise. On ne perd plus de temps à valider manuellement chaque annonce.',
                    name: 'Pierre L.',
                    role: 'Garage familial, Berne',
                  },
                  {
                    quote:
                      "J'utilise CarPulse depuis 3 mois et mes marges moyennes ont clairement progressé. Je recommande à tous ceux qui font de l'achat-revente sérieusement.",
                    name: 'Nadia F.',
              ...
              <div
                key={colIdx}
                className={`relative overflow-hidden md:h-[min(480px,70vh)] md:max-h-[560px] ${col.className}`}
              >
                <div
...
const TRUST_ITEMS = [
  {
    ...
    dMobile: "Remboursé si l'outil ne vous apporte pas de valeur.",
    icon: ShieldCheck,
  },
  {
    t: 'Paiement sécurisé',
    d: 'Transactions protégées et\nabonnement simple à gérer.',
    dMobile: 'Transactions protégées et abonnement simple à gérer.',
    icon: CreditCard,
  },
  {
    t: 'Sans engagement',
    d: 'Vous pouvez arrêter à tout\nmoment',
    dMobile: 'Arrêtez votre abonnement à tout moment.',
    icon: CircleCheck,
  },
] as const;
        ...
        <div className="mt-10 sm:mt-16">
          ...
          <div className="space-y-2.5 md:hidden">
            {TRUST_ITEMS.map(({ t, dMobile, icon: Icon }) => (
              <div
                key={t}
                className="flex items-center gap-3.5 rounded-2xl border border-[#FFE4CC]/60 bg-[#FFF9F2] px-4 py-3.5"
              >
            ...
            <div className="grid gap-8 md:grid-cols-3">
              {TRUST_ITEMS.map(({ t, d, icon: Icon }) => (
                <div key={t} className="text-center md:text-left">
                  <div className="inline-flex text-[#FF7A22]">
...
const FAQ_ITEMS: { question: string; answer: string }[] = [
  {
    question: "Combien d'opportunités vais-je recevoir ?",
    answer:
      'Le nombre varie selon votre plan et vos filtres. En moyenne, nos utilisateurs reçoivent entre 20 et 40 opportunités qualifiées par mois, toutes scorées et analysées automatiquement.',
  },
  {
    question: "D'où proviennent les annonces analysées par CarPulse ?",
    answer:
      'CarPulse agrège en continu les annonces des principales plateformes automobiles publiques (AutoScout24, Anibis, Comparis, etc.) et analyse chaque véhicule dès sa mise en ligne.',
  },
  {
    question: 'Comment CarPulse détermine si un véhicule est rentable ?',
    answer:
      "Notre algorithme compare chaque annonce à des milliers de transactions similaires (marque, modèle, année, kilométrage, options) et calcule un écart au prix du marché. Un mini-rapport vous explique le potentiel de marge en quelques secondes.",
  },
  {
    question: 'CarPulse est-il destiné aux particuliers ?',
    answer:
      "Non, CarPulse est conçu pour les professionnels de l'automobile : garages, marchands, revendeurs indépendants et importateurs. L'outil est optimisé pour l'achat-revente à fort volume.",
  },
  {
    question: 'Puis-je arrêter mon abonnement à tout moment ?',
    answer:
      'Oui, tous nos plans sont sans engagement. Vous pouvez suspendre ou annuler votre abonnement à tout moment depuis votre espace, sans frais cachés.',
  },
  {
    question:
      'Que se passe-t-il si les opportunités ne sont pas pertinentes pour mon activité ?',
    answer:
      "Vous pouvez affiner vos filtres à tout moment (marque, kilométrage, budget, zone géographique). Si l'outil ne vous apporte pas de valeur, nous appliquons notre garantie satisfaction avec remboursement.",
  },
];
...
export function Section10FaqCta() {
  return (
    <>
      <section id="faq" className="bg-white py-10 sm:py-20 lg:py-24">
        <Container>
          <div className="mx-auto max-w-[720px]">
            <h2 className="text-center text-[22px] font-bold leading-tight tracking-tight text-[#1A1A1A] sm:text-4xl">
              <span className="text-[#FE6C0E]">Questions</span>{' '}
              <span className="max-sm:block">fréquentes</span>
            </h2>

            <div className="mt-6 space-y-2.5 sm:mt-12 sm:space-y-3">
              {FAQ_ITEMS.map((item, i) => (
                <FaqAccordion key={i} question={item.question}>
                  {item.answer}
                </FaqAccordion>
              ))}
            </div>
